...
面向通用软件开发工作的本地桌面工具箱，基于 Electron、Vue、TypeScript、SiliconFlow 和 SQLite。

## 功能概览

- JSON 工具：格式化、压缩、校验和路径查询。
- API 测试：环境变量、请求集合、请求历史、cURL 复制和 AI 分析。
- 项目工作区：维护常用项目目录、标签、说明、最近使用时间、项目列表导入导出和项目数据包导入导出。
- Git 助手：状态、日志、diff、文件级暂存、AI 变更说明和 Commit Message。
- 地理数据：GeoJSON 体检、字段模板、几何类型统计、坐标范围、必填属性检查和项目历史。
...
二期从项目维度的能力建设开始。当前已新增项目工作区，用于登记常用本地项目目录、业务标签和说明；AI 历史、API 请求集合、Git 默认目录和地理数据体检历史已支持项目关联，项目工作区可导入导出项目列表和项目数据包。

三期继续增强项目包迁移、地理数据文件工作流和 AI 复用能力。当前已支持项目包导入预览与冲突策略、GeoJSON 文件导入、地理体检报告导出，以及从 AI 历史中心创建 Prompt 模板。

三期还新增项目 API 自动发现：选择项目后由 Electron 主进程异步扫描后端 Spring Controller 和前端 fetch / axios 调用，按 Controller 或来源文件分组预览，再导入为项目级 API 请求集合。前端只负责发起扫描和展示结果，不直接多线程读取项目文件。
...
- 清理 AI 历史、API 请求历史、API 请求集合、地理体检历史、自定义 Prompt 模板。
...
- 导出 / 导入 API 请求集合 JSON。
- 在项目工作区导入 / 导出项目列表 JSON。
- 在项目工作区导入 / 导出项目数据包 JSON。
