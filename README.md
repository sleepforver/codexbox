...
当前公开版本命名为 **v0.1.0 Beta**。这个版本适合作为第一版公测版试用，不建议按长期稳定版分发给非技术用户。当前开发分支已经进入 **v0.2.0 项目管理开发快照**，主线从“工具集合”升级为“项目管理型 AI 工作台”。
...
- 核心模块已经形成基础闭环，但仍缺少完整的 Electron E2E 回归、正式自动更新和大规模用户数据迁移验证。
- 下载安装包后建议先在测试项目或非关键仓库中试用。

不建议使用的命名：

- `v1.0.0`：稳定性和自动更新条件还不够。
- `正式版`：当前更适合 beta / 公测版定位。
- `内测版`：官网和 R2 下载已经具备公开试用入口，内测语义偏保守。

### v0.2.0 项目管理开发快照

第二版开发分支的目标是把应用从通用 AI 工具箱升级为项目管理型 AI 工具箱。当前主流程已经围绕项目组织：

- `项目总览` 聚合项目资料、任务统计、Git 摘要、API 资产和 AI 活动。
- `项目任务` 支持需求、Bug、优化、重构、文档和发布任务，并能关联 AI 输出和任务文件。
- `Agent工作站` 支持为项目配置项目经理、架构、开发、测试、审查等 Agent，并围绕项目任务生成可追踪的协同工作流草案。
- `AI开发` 可以载入项目与任务上下文，生成开发方案、测试建议和提交说明草稿。
- `代码审查` 可以审查 unstaged diff、staged diff、指定文件或粘贴内容，并把结果写入项目/任务历史。
- `项目知识库` 可以沉淀项目资料、AI 结论、API 记录和任务备注，并保留来源信息。
- `项目数据包` 已覆盖项目资料、项目任务、项目知识库、AI 历史和 API 请求集合，支持覆盖、跳过和另存新项目。
...
当前发布包暂未进行代码签名，Windows 可能显示 SmartScreen 或未知发布者提示。请优先从官网下载安装包，并用官网展示的文件大小和 SHA256 与本地下载文件核对；签名证书会在后续预算允许后再纳入稳定版发布链路。
...
- `项目总览`：聚合当前项目资料、任务、Git、API 和 AI 活动。
- `项目任务`：管理需求、Bug、优化、重构、文档和发布任务。
- `Agent工作站`：维护项目内多 Agent 分工，并把目标或任务编排为工作流。
- `项目知识库`：沉淀项目资料、AI 结论、接口说明和任务备注。
- `AI开发`：基于项目和任务上下文生成方案、测试建议和提交说明。
- `代码审查`：审查 diff、指定文件或粘贴内容，并可关联项目任务。
- `API管理`：发送 HTTP 请求、保存集合、导入接口、分析响应并关联任务。
- `Git工作流`：查看状态、日志、diff，暂存文件并生成提交说明。
- `项目设置`：维护项目资料、命令、重要目录和项目数据包。
...
项目数据包用于迁移项目相关数据，包括项目资料、项目任务、项目知识库、AI 历史和 API 请求。

- `导出项目包`：导出当前项目相关数据。
- `导入项目包`：导入前显示项目、任务、知识条目、AI 历史、API 请求数量和冲突项目。
- 冲突策略：支持覆盖、跳过和另存新项目。

## 项目总览与任务

`项目总览` 是第二版的主入口。选择项目后，页面会显示项目路径状态、技术栈、任务统计、最近任务、Git 摘要、AI 活动和 API 资产。路径失效或没有项目时，页面会给出明确空状态。

`项目任务` 用于维护项目内的需求、Bug、优化、重构、文档和发布任务。每个任务都归属于一个项目，包含标题、描述、类型、优先级、状态、创建时间、更新时间和完成时间。任务页支持：

- 按状态、类型和关键词筛选。
- 上传任务文件并在任务上下文中引用。
- 从任务发起 AI 开发方案、测试建议和提交说明草稿。
- 查看当前任务关联的 AI 历史输出。

`Agent工作站` 用于把项目管理动作拆给多个 AI Agent。每个项目会生成默认 Agent 队列，也可以手动维护 Agent 名称、角色、模型平台、模型、职责范围和系统提示词。工作站支持选择项目任务或输入独立目标，生成包含各 Agent 指令的协同工作流草案，并可把工作流提示发送到 `AI开发` 页面继续生成方案。

## 项目知识库

`项目知识库` 用于保存可追溯的项目结论和资料条目。条目支持标题、内容、来源类型、来源 ID 和收藏状态。AI 历史中心可以把重要输出沉淀为知识条目，后续可按项目检索和复用。

知识库页还提供项目内统一搜索，结果限定在当前项目内，覆盖项目资料、项目任务、知识条目、AI 历史和 API 请求，便于从一个入口追踪项目上下文。
...
- [ ] 安装版可以安装、启动、关闭和卸载。
- [ ] 便携版可以启动并正常读写用户数据。
- [ ] 官网和 README 已说明当前安装包未代码签名，Windows 可能出现 SmartScreen 或未知发布者提示。
- [ ] 首次启动会自动创建数据库。
- [ ] 无 API Key 时页面提示清晰。
