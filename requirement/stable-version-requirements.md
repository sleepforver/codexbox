...
但该版本仍按 beta / 公测版定位，尚未具备长期稳定版所需的发布可复现性、自动更新、完整 E2E 回归、首次启动引导和大规模数据迁移验证能力。由于当前暂不申请 exe 代码签名证书，签名分发调整为预算允许后的后续增强，不作为当前稳定版方案的阻塞项。
...
- 处理更新校验、失败回滚和更新说明展示。
- 当前暂不申请 exe 代码签名证书；需要在官网、README 和发布说明中明确未签名风险，并提供 SHA256 校验方式。
- 增加更新失败后的手动下载兜底入口。
...
- 未签名安装包的 Windows SmartScreen / 未知发布者风险已在官网和 README 明示，用户可通过 SHA256 核对下载文件。
...
- 当前仍未接入静默下载安装、差分自动应用和真实 v0.x -> v1.0.0 升级回归；签名证书校验因预算原因暂缓，后续证书就绪后再纳入可信分发增强。
...
- 已扩展 `scripts/verify-playwright-electron.mjs`，覆盖首次启动引导展示、跳转设置页、跳过、恢复和截图产物。
...
- 官网增加系统要求、文件大小、SHA256、Beta/Stable 状态、更新策略和未签名安装包风险说明。
